...
while (true)
{
    ...
    {
        //Getting the storage id
        var storageId = await loopringTradeService.GetNextStorageId(settings.LoopringApiKey2, settings.LoopringAccountId2, nftTokenId);
        var storageId2 = await loopringTradeService.GetNextStorageId(settings.LoopringApiKey, settings.LoopringAccountId, 1);
        Console.WriteLine($"Storage id: {JsonConvert.SerializeObject(storageId, Formatting.Indented)}");
        Console.WriteLine($"Storage id2: {JsonConvert.SerializeObject(storageId2, Formatting.Indented)}");
    ...
    Utils.ParseHexUnsigned("0x0000000000000000000000000000000000000000")
        ...
        //Generate the poseidon eddsa signature
        Eddsa eddsa = new Eddsa(makeOrderPoseidonHash, settings.LoopringPrivateKey2);
        string makerEddsaSignature = eddsa.Sign();
        nftMakerOrder.eddsaSignature = makerEddsaSignature;

        var nftMakerTradeValidateResponse = await loopringTradeService.SubmitNftTradeValidateOrder(settings.LoopringApiKey2, nftMakerOrder, makerEddsaSignature);
    ...
    Utils.ParseHexUnsigned("0x0000000000000000000000000000000000000000")
        ...
        var nftTakerTradeValidateResponse = await loopringTradeService.SubmitNftTradeValidateOrder(settings.LoopringApiKey, nftTakerOrder, takerEddsaSignature);
